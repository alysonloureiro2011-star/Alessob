...
def save_instagram_auth(
    ...
    *,
    ...
    resolved_token = token or config.ig_token
    resolved_user_id = str(user_id or config.ig_id) if (user_id or config.ig_id) else None

    existing_user_id = existing.get("user_id") or existing.get("ig_id")
    unchanged_identity = (
        existing.get("token") == resolved_token
        and str(existing_user_id) == str(resolved_user_id)
    )
    ...
    payload = {
        ...
        "saved_at": existing.get("saved_at") if unchanged_identity and existing.get("saved_at") else datetime.now().isoformat(),
        "meta": merged_meta,
    }
...
def sync_instagram_token_sources(
    ...
    *,
    ...
    existing_user_id = stored.get("user_id") or stored.get("ig_id")
    changed_identity = (
        stored.get("token") != unified_token
        or str(existing_user_id) != str(unified_user_id)
    )

    payload = None
    if persist and (unified_token or unified_user_id) and (changed_identity or not stored):
