...
class MultimodalReelQA:
    ...
    def run(
        ...
        *,
        ...
        natural_score = 8.1 if naturalism.get("naturalism_state") == "naturalism_engine_ready" else 6.0
        global_score = round((visual_score + audio_score + rhythm_score + natural_score) / 4.0, 2)
        approved = global_score >= 8.2
        ...
        return MultimodalQAResult(
            ...
            approved=approved,
            notes=[f"global_score={global_score}", f"approved={str(approved).lower()}"]
        ).to_dict()
