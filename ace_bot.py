...
# ==========================================================
# ACE Ω V6000+ CONSOLIDADO FINAL - VACINA + EVOLUTIVO
# ==========================================================
import os, time, datetime, gc, requests, threading, random, re, json, sqlite3
import numpy as np
from collections import Counter
    ...
    def run():
        while True:
            try:
                ...
                threading.Thread(target=lambda: safe_flask_run(app,port), daemon=True).start()
            threading.Event().wait(interval)
    threading.Thread(target=run, daemon=True).start()

# ==========================================================
# 🚀 PATCH VACINA - INTEGRAÇÃO TOTAL
# ==========================================================
...
def sociocultural_meter():
    return {"trend_level": random.uniform(0.7,1.3), "hora": datetime.datetime.now().hour}
...
def ciclo_vacina(app, OUT_PATH, GENES):
    try:
        public_url=f"https://{os.environ.get('RENDER_EXTERNAL_URL','localhost')}"
        print("ACE ONLINE VACINA:", public_url)
        while True:
            agora=datetime.datetime.now()
            if agora.hour in [6,8,10,12,14,16,18,20,22] and agora.minute==0:
                tema,hook,score=omega_brain()
        ...
        print("ERRO CICLO VACINA:", e)

def start_vacina(app, OUT_PATH, GENES):
    threading.Thread(target=lambda: safe_flask_run(app,5000), daemon=True).start()
    safe_watchdog(app,5000)
    threading.Thread(target=lambda: ciclo_vacina(app,OUT_PATH,GENES), daemon=True).start()

# ==========================================================
# 🚀 START FINAL
# ==========================================================
if __name__=="__main__":
    start_vacina(app, OUT_PATH, GENES)
        except Exception as e:
